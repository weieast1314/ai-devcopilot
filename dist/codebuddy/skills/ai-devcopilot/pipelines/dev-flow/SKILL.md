...
1. **先计划，后执行**：进入实现阶段前，必须先完成计划文档。
2. **计划待确认**：计划生成后默认暂停，等待用户明确回复"确认计划，开始执行"后才能改代码。
3. **只按当前计划执行**：实现阶段仅允许执行当前计划中的任务，不得擅自扩大范围。
4. **偏差先更新计划**：如果发现遗漏、依赖变化或方案偏差，先同步更新计划，再继续执行。
5. **完成一项，汇报一项**：每完成一个任务，都要汇报任务状态、修改文件、验证结果和下一步。
