...
```bash
# 查找已存在的相关分支
git branch -a | grep -E "(feat|fix|refactor)/.*${issue_id}"
```
...
请选择从哪个分支创建新分支：
1) 从 uat 分支创建（线上分支）[默认] - 推荐，基于最新的线上代码
2) 从 当前 分支创建 - 基于当前所在的分支创建
3) 从 其他分支创建（手动输入） - 指定其他分支名称
...
| 选项 | 说明 | 适用场景 |
|------|------|----------|
| 1) uat | 从线上分支创建，推荐用于新功能开发 | 标准开发流程 |
| 2) 当前分支 | 从当前所在分支创建 | 延续当前工作 |
| 3) 其他分支 | 手动输入分支名称 | 特殊场景 |
