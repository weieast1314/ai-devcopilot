...
│                     Composites (组合)                        │
│  封装常用流程，组合多个原子 Skill                              │
│  示例: feishu-doc-to-branch, text-to-branch                  │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
```
用户输入 → input-detect (输入检测)
              ├── 飞书链接 → feishu-doc-fetch → requirement-parse
              └── 文字描述 → requirement-extract → requirement-parse
```
...
```text
继续刚才的任务：当前分支 feature/23181-login，计划已确认，已经完成接口层，接下来继续执行 Service 和测试验证
```
...
│   ├── composites/                 # 组合 Skill 层（5 个）
│   ├── pipelines/                  # Pipeline 层（2 个）
│   └── registry/
│       └── skills-registry.yml
├── templates/
│   ├── agent/                      # 编辑器 Agent 模板
